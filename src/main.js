...
import { SignerPermissionRequestV1, ThirdwebSDK } from "@thirdweb-dev/sdk";
...
k.scene("wallet", (levelIdx) => {
  ...
  const level = k.addLevel(levels[levelIdx], {
    ...
    pos: k.vec2(64, 64),
    tiles: {
      "=": () => [k.sprite("grass"), k.area(), k.body({ isStatic: true }), k.anchor("center")],
      "-": () => [k.sprite("steel"), k.area(), k.body({ isStatic: true }), k.anchor("center")],
      $: () => [k.sprite("pubkey"), k.area(), k.anchor("center"), "key"],
      "*": () => [k.sprite("prikey"), k.area(), k.anchor("center"), "key"],
      "@": () => [k.sprite("hero"), k.area(), k.body(), k.anchor("center"), "player"],
      "|": () => [
        k.sprite("door"),
        k.area(),
        k.body({ isStatic: true }),
  ...
  function addDialog() {
    const h = 160;
    const pad = 16;
    const bg = k.add([k.pos(0, k.height() - h), k.rect(k.width(), h), k.color(0, 0, 0), k.z(100)]);
    const txt = k.add([
      k.text("", {
        width: k.width(),
      }),
...
k.scene("wallet-win", () => {
  k.add([k.sprite("wallet-win"), k.pos(k.width() / 2, k.height() / 2), k.anchor("center")]);
